	...
	public void run() {
		try {
			...
			try {
				...
				if(user1Choice==1) {
					System.out.println("L'utilisateur souhaite cr�er un compte");
					...
					System.out.println("L'utilisateur souhaite se connecter � un compte existant");
					...
					while(connectionAccepted == false) {
						...
						output.writeObject("Connection refus�e");
				...
				while(changed != 5) {
					...
					if(changed == 1) {
						...
						if(mcs.dbc.verifUserExists(pseudoContact)) {
							...
							output.writeObject(true); // on envoi r�ussite a l'utilisateur
						}else {
